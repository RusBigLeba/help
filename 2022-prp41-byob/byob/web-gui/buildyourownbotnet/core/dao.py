...
class SessionDAO:
    ...
    def handle_session(self, session_dict):
        ...
        if not session_dict.get('uid'):
            ...
            session_dict['joined'] = datetime.now()

        # upddate session status to online
        session_dict['online'] = True
        session_dict['last_online'] = datetime.now()
...
class TaskDAO:
    ...
    def handle_task(self, task_dict):
        ...
        if not task_dict.get('uid'):
            identity = str(
                str(task_dict.get('session')) + str(task_dict.get('task')) + datetime.now().__str__()).encode()
            task_dict['uid'] = hashlib.md5(identity).hexdigest()
            task_dict['issued'] = datetime.now()
            task = Task(**task_dict)
            db.session.add(task)
...
class FileDAO:
    ...
    def rm_user_file(self, username, filename):
        """
        Delete the selected user payload from database.
        ...
        if user:
            file = db.session.query(self.model).get(filename)
            try:
                os.remove(os.path.abspath(
                    os.path.join(os.path.dirname(__file__), os.pardir, 'output', user.username, 'files',
                                 filename)))
            except FileNotFoundError:
                pass
            db.session.query(self.model).filter_by(filename=filename).delete()
            db.session.commit()
            return file
...
class PayloadDAO:
    ...
    def rm_user_payload(self, username, filename):
        """
        Delete the selected user payload from database.
        ...
        user = self.user_dao.get_user(username=username)
        if user:
            payload = db.session.query(self.model).get(filename)
            try:
                if filename[-3:] == '.py':
                    os.remove(os.path.abspath(
                        os.path.join(os.path.dirname(__file__), os.pardir, 'output', user.username, 'src',
                                     filename)))
                    os.remove(os.path.abspath(
                        os.path.join(os.path.dirname(__file__), os.pardir, 'modules', 'payloads', filename[6:])))
                    os.remove(os.path.abspath(
                        os.path.join(os.path.dirname(__file__), os.pardir, 'modules', 'stagers', filename[6:])))
                elif filename[-4:] == '.exe' or filename[-4:] == '.EXE':
                    os.remove(os.path.abspath(
                        os.path.join(os.path.dirname(__file__), os.pardir, 'output', user.username, 'src', 'dist',
                                     'windows',
                                     filename)))
                    os.remove(os.path.abspath(
                        os.path.join(os.path.dirname(__file__), os.pardir, 'modules', 'payloads',
                                     filename[-7:-4] + '.py')))
                    os.remove(os.path.abspath(
                        os.path.join(os.path.dirname(__file__), os.pardir, 'modules', 'stagers',
                                     filename[-7:-4] + '.py')))
                else:
                    os.remove(os.path.abspath(
                        os.path.join(os.path.dirname(__file__), os.pardir, 'output', user.username, 'src', 'dist',
