...
class Loader(object):
    ...
    def load_module(self, name):
        ...
        final_src = None
        try:
            log(level='debug', info="Trying to import '%s' as package from: '%s/__init__.py'" % (name, package_url))
            package_src = None
            if self.non_source:
        ...
        if final_src == None:
            package_url = self.base_url + 'site-packages/' + '%s/__init__.py' % name.replace('.', '/')
            try:
                ...
                log(level='info', info="[-] '%s' is not a package (%s)" % (name, str(e)))
        if final_src == None:
            package_url = self.base_url + 'site-packages/' + '%s.py' % name.replace('.', '/')
            try:
                log(level='debug', info="Trying to import '%s' as package from: '%s'" % (name, package_url))
                package_src = None
                if self.non_source:
                ...
                log(level='info', info="[-] '%s' is not a package (%s)" % (name, str(e)))
        if final_src == None:
            package_url = self.base_url + 'site-packages/' + name.split('.')[0] + '/' + '%s.py' % name.split('.')[-1]
            try:
                log(level='debug', info="Trying to import '%s' as package from: '%s'" % (name, package_url))
                package_src = None
                if self.non_source:
    ...
    def __fetch_compiled(self, url):
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                module_src = marshal.loads(
                    module_compiled[12:])  # Strip the .pyc file header of Python 3.3 and onwards (changed .pyc spec)
                return module_src
            except ValueError:
                pass
        except IOError as e:
