...
def payload_generate():
    ...
    try:
        ...
        flash('Error: compilation timed out or failed. Please open an issue for help.')
        print("Exception in api.routes.payload.payload_generate: " + str(e))
    return redirect(url_for('main.payloads'))


@payload.route("/api/payload/delete", methods=["POST", "GET"])
@login_required
def payload_delete():
    """Delete the chosen scripts"""
    username = request.args.get('username')
    filename = request.args.get('filename')
    payload_dao.rm_user_payload(username, filename)
    flash('Successfully deleted payload: ' + filename, 'success')
    return redirect(url_for('main.payloads'))
